...
def make_samples(in_queue, out_queue):
    ...
    while True:
        ...
        if "nn_verification" in args.src_folder:
            m.setRealParam('numerics/feastol', 1e-04)
        if "mmtc" in args.src_folder:
...
if __name__ == '__main__':
    ...
    #     instances_train = glob.glob('data/instances/setcover/train2/*.lp')
    #     instances_valid = glob.glob('data/instances/setcover/valid2/*.lp')
    #     instances_test = glob.glob('data/instances/setcover/test2/*.lp')
    #     out_dir = 'data/samples/setcover/weimin'
    #     train_size = 10*len(instances_train)
    #     valid_size = 10*len(instances_valid)
    #     test_size = 10*len(instances_test)
    ...
    instances = glob.glob(args.src_folder+"/*")
    print(instances)
    instances = [item for item in instances if (item.split(".")[-1] in ["lp","mps"])]
    out_dir = args.dst_folder
    size = 10*len(instances)
    

    print(f"{len(instances)} train instances for {instances} samples")
    ...
    if "train" in args.src_folder:
        rng = np.random.RandomState(args.seed)
    if "val" in args.src_folder:
        rng = np.random.RandomState(args.seed + 1)
    if "test" in args.src_folder:
        rng = np.random.RandomState(args.seed + 2)
    #rng = np.random.RandomState(args.seed)
    collect_samples(instances, out_dir, rng, size,
